...
public partial struct PathFindingSystem : ISystem
{
    ...
    public partial struct PathFindingJob : IJobEntity
    {
        ...
        [BurstCompile]
        public void Execute(RefRW<MovementComponent> _movement, RefRO<LocalTransform> _transform)
        {
            ...
            while(endNode.parentIndex!=-1 && endNode.parentIndex!= pathNodes[PathNode.CalculateIndex(startposition.x, startposition.y, gridsize.x)].index)
            {
                endNode = pathNodes[endNode.parentIndex];
            }
            if (endNode.parentIndex!=-1)
            {
                
                _movement.ValueRW.NextPosition = grid.GetCellPosition(endNode.x-grid.GridSize.x/2,endNode.y - grid.GridSize.y / 2);
            }
